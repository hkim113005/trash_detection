...
def convert_coco_json(json_dir="../coco/annotations/", use_segments=False, cls91to80=False, target_dir="target"):
    """Converts COCO JSON format to YOLO label format, with options for segments and class mapping."""
    save_dir = make_dirs(target_dir)  # output directory
    coco80 = coco91_to_coco80_class()

    # Import json
    for json_file in sorted(Path(json_dir).resolve().glob("instances_filtered.json")):
...
if __name__ == "__main__":
    ...
    if source == "COCO":
        convert_coco_json(
            "/work/home/jae/trash_detection/annotations_seg/26/annotations",  # directory with *.json
            use_segments=True,
            cls91to80=True,
        )
